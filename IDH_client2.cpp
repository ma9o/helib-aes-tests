...
int main(int argc, char **argv)
{
  ...
  std::string doublyEncryptedFileName = "input.bin"; // HE(AES(PIIs)) coming from IDVP
  amap.arg("doublyEncryptedFileName", doublyEncryptedFileName, "HE(AES(PIIs)) filename coming from the IDVP");

  std::string secretKeyFileName = "secretKey.bin"; // the key from IDH_client1
  amap.arg("secretKeyFileName", secretKeyFileName, "The secret key used by IDH to encrypt the PIIs at the beginning");

  std::string outfileName = "output.bin"; // AES(PIIs)
  amap.arg("outfileName", outfileName, "AES(PIIs) filename");
  ...
  // Read secret key from step 1
  std::ifstream secretKeyFile (secretKeyFileName);
  readSecKeyBinary(secretKeyFile, secretKey);
  secretKeyFile.close();
  ...
  Ctxt tmpCtxt(secretKey);
  doublyEncrypted.resize(stoi(inputSize), tmpCtxt);

  // Read HE(AES(PIIs)) from IDVP
  std::ifstream doublyEncryptedFile (doublyEncryptedFileName);
  doublyEncrypted.data()->read(doublyEncryptedFile);
  doublyEncryptedFile.close();
  ...
  cout << uint8_to_hex_string(aesCtxt.data(), aesCtxt.length()) << endl;
